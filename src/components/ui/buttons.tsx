...
export function Buttons({ route }: { route: string }) {
  ...
  const buttons = [
    {
      ...
      showDot: false,
      component: (
        <button className="inline-flex h-12 animate-shimmer items-center justify-center rounded-md border border-slate-800 bg-[linear-gradient(110deg,#000103,45%,#1e2631,55%,#000103)] bg-[length:200%_100%] px-6 font-medium text-slate-400 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50 fixed bottom-12 right-15" 
  ...
  //   const textToCopy = button.code || reactElementToJSXString(button.component);
  //   if (textToCopy) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-full max-w-7xl mx-auto gap-10">
        ...
        {buttons.map((button, idx) => (
          <div key={idx} className="flex flex-col items-center space-y-2">
            {button.component}
            {/* <span className="text-sm text-gray-500">{button.name}</span> */}
          </div>
        ))}
      </div>
    </div>
